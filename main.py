...
def getBestOrf(orfWithValueArray):
    maxElement = []
    bestValue = -57 #note the lowest match value is -56 as all our promoters are 8 base pairs long
...
def main(fileName, fileNumber):
    if(False): #replace with len(argv) != 2 when done testing
        print("Usage:  python3 main.py <fasta filename>.fasta")
    else:
        dna1 = translate(fileName) #replace with argv[1] when done testing
        ...
        for elements in possibleReadingFrameArr1:
            orfWithValue1.append(ORF(elements, getPromoterValue(dna1[max(0,elements.startIndex - 200):max(0, elements.startIndex -50 + 1)]),dna1[elements.startIndex:elements.stopIndex+3], 1, elements.startIndex%3 + 1 ))
        ...
        if(len(finals) > 0):
            ...
            print(bestframe)
                ...
                #print(fileNumber, element.dnaStrand, element.possibleReadingFrame.startIndex, element.possibleReadingFrame.stopIndex + 3, len(dna1) - 2000)
        else:
