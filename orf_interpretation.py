...
def fuzzyMatch(str1, str2):
    ...
    for row in range(len(workstr1)-1):
        for col in range(len(workstr2) -1):
            possibleChoices = [(matchArr[row][col+1] + gapPenalty), (matchArr[row+1][col] + gapPenalty), ((matchArr[row][col] + matchReward) if (workstr1[row + 1] == workstr2[col + 1]) else (matchArr[row][col] + matchPenalty))]
